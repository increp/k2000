...
| **v7** | **Source & DSP block library** | Sources as blocks. The **Bernie oscillator** is a per-waveform **mini-mixer**: each classic waveform (saw / square / triangle / sine) has its **own level slider** *and* there's a **wavetable** slot, all summed into a **morphed composite waveform** (e.g. partial triangle + partial saw) — not a single-waveform selector. Plus Summit-style drift/sync/FM, **KVA**, noise; and DSP blocks (mixer, ring mod, shaper/drive variants, EQ, timbre filters, hard sync). UI: dynamic per-block knob-clusters + the per-osc waveform mixer. |
