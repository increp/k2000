...
- **Locked decisions** (from the register): **full stereo throughout** · **64 voices,
  always, at every OS tier** (final 2026-07-02; the binding budget is 64 × os8 realtime —
  register Q2/Q23) · spine + modulation **per-Layer** · **synth-only** sources now
  (sample/keymap arrive later).
...
- **Performance is a gate** — **64 voices × full stereo × graph DSP at EVERY OS tier** must
  render realtime on the target machine; profiling is a release gate (numbers: register Q23;
  the user plays at os8, so 64 × os8 is the binding case).
...
(64 at every OS tier, register Q2), then write an ADR.
