...
- **D1 — Stereo render path.** `Voice::render`, `VoiceManager::renderBlock`, and the processor scratch become **stereo** (`L`,`R`). The per-voice source/graph stays mono (sources go stereo in later phases); the spine receives the mono voice signal on both channels and is free to diverge L/R later. The processor renders into a stereo scratch instead of copying mono to all channels. *Note: this is a deliberate departure from the hardware — the real Summit filter/drive are mono, splitting to L/R only at the VCA (register Q1). Full-stereo-throughout was chosen knowingly; the spine filter is stereo here.*
