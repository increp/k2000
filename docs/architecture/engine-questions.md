...
| Q12 | **Filter oversampling policy at 256 voices.** The [Huggett report](huggett-filter.md) wants the nonlinear path oversampled (2× default). Per-voice OS ×256 is costly — do we oversample per voice (conditional on drive/resonance?), use cheaper per-voice anti-aliasing, or a shared/SIMD scheme? | Largest v5 CPU risk; gates fidelity-vs-budget. Depends on Q11. | v5 | 🔴 |
| Q13 | **Dual-section vs single SVF at v5.** Ship the lineage-faithful **two linked 12 dB sections + separation** from v5, or a single TPT SVF first with the dual section as a later enhancement? | Core fidelity vs v5 scope/cost. | v5 | 🔴 |
| Q14 | **Filter mode set exposed at v5.** Full nine-mode set (LP/BP/HP + dual combinations + OSCar LP/HP/BP separation law), or Summit dual modes first and OSCar modes as stretch? | Scope of v5 filter UI + DSP. | v5 | 🔴 |
| Q15 | **White-box OTA vs gray-box tanh.** v5 starts gray-box (tanh saturators, calibrated laws). How far toward component-accurate OTA modeling do we ever go, and when? | Fidelity ceiling vs effort; later phase. | v5 (start) / later | 🟡 |
| Q16 | **Filter promotion + preset migration.** v5 promotes the optional `SVFFilter` palette block into the constant spine with new params. How does the cumulative preset shim migrate v1–v4 presets onto the new spine filter? | Preset stability across the v5 promotion. | v5 | 🔴 |
