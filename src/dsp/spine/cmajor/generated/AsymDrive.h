...
//==============================================================================
/// Auto-generated C++ class for the 'AsymDrive' processor
///
...
struct AsymDrive
{
    ...
    static constexpr std::string_view name = "AsymDrive";
    ...
    //==============================================================================
    static constexpr uint32_t numInputEndpoints  = 4;
    ...
    static constexpr EndpointInfo inputEndpoints[] =
    {
        ...
        { 2,  "drive01",     EndpointType::event  },
        { 3,  "biasFixed",   EndpointType::event  },
        { 4,  "maxDriveDb",  EndpointType::event  }
    };

    static constexpr EndpointInfo outputEndpoints[] =
    {
        { 5,  "out",  EndpointType::stream }
    };
    ...
    struct AsymDrive_State
    {
        int32_t _currentFrame = {};
        _AsymDrive_State _state;
    };
    ...
    {
        if (endpointHandle == 5) { std::memcpy (reinterpret_cast<char*> (dest), std::addressof (cmajIO.out), 4 * numFramesToCopy); std::memset (reinterpret_cast<char*> (std::addressof (cmajIO.out)), 0, 4 * numFramesToCopy); return; }
    ...
    AsymDrive_State cmajState = {};
    AsymDrive_IO cmajIO = {};

    //==============================================================================
    void _sendEvent_drive01 (AsymDrive_State& _state, float value) noexcept
    {
        _AsymDrive__drive01 (_state._state, value);
    }

    void _AsymDrive__drive01 (_AsymDrive_State& _state, float v) noexcept
    {
        _state.d01 = v;
        _state.dirty = true;
    }

    void _sendEvent_biasFixed (AsymDrive_State& _state, float value) noexcept
    {
        _AsymDrive__biasFixed (_state._state, value);
    }

    void _AsymDrive__biasFixed (_AsymDrive_State& _state, float v) noexcept
    {
        _state.biasF = v;
        _state.dirty = true;
    }

    void _sendEvent_maxDriveDb (AsymDrive_State& _state, float value) noexcept
    {
        _AsymDrive__maxDriveDb (_state._state, value);
    }

    void _AsymDrive__maxDriveDb (_AsymDrive_State& _state, float v) noexcept
    {
        _state.maxDb = v;
        _state.dirty = true;
    }

    void _initialise (AsymDrive_State& _state, int32_t& processorID, int32_t sessionID, double frequency) noexcept
    {
        _AsymDrive___initialise (_state._state, processorID, sessionID, frequency);
    }

    void _AsymDrive___initialise (_AsymDrive_State& _state, int32_t& processorID, int32_t sessionID, double frequency) noexcept
    {
    ...
    void _advance (AsymDrive_State& _state, AsymDrive_IO& _io, int32_t _frames) noexcept
    {
        _AsymDrive_IO  ioCopy;
        ...
        for (;;)
        {
            ...
            ioCopy = _AsymDrive_IO {};
            ioCopy.in = _io.in[_state._currentFrame];
            _AsymDrive__main (_state._state, ioCopy);
    ...
    void _AsymDrive__main (_AsymDrive_State& _state, _AsymDrive_IO& _io) noexcept
    {
        for (;;)
        {
            if (_state.dirty)
            {
                _AsymDrive__recompute (_state);
            }
            _io.out = (_io.out + (_state.comp * intrinsics::tanh ((_state.gain * _io.in) + _state.bias)));
            return;
        }
    }

    void _AsymDrive__recompute (_AsymDrive_State& _state) noexcept
    {
        float  dB;
        float  full;
