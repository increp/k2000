...
//==============================================================================
/// Auto-generated C++ class for the 'NlSvf' processor
///
...
struct NlSvf
{
    ...
    static constexpr std::string_view name = "NlSvf";
    ...
    NlSvf_State cmajState = {};
    NlSvf_IO cmajIO = {};

    //==============================================================================
    void _sendEvent_cutoffHz (NlSvf_State& _state, float value) noexcept
    {
        _NlSvf__cutoffHz (_state._state, value);
    }

    void _NlSvf__cutoffHz (_NlSvf_State& _state, float v) noexcept
    {
        _state.cutoff = v;
        _state.dirty = true;
    }

    void _sendEvent_resonance (NlSvf_State& _state, float value) noexcept
    {
        _NlSvf__resonance (_state._state, value);
    }

    void _NlSvf__resonance (_NlSvf_State& _state, float v) noexcept
    {
        _state.res = v;
        _state.dirty = true;
    }

    void _sendEvent_resSat (NlSvf_State& _state, float value) noexcept
    {
        _NlSvf__resSat (_state._state, value);
    }

    void _NlSvf__resSat (_NlSvf_State& _state, float v) noexcept
    {
        _state.rsat = v;
    }

    void _sendEvent_tap (NlSvf_State& _state, int32_t value) noexcept
    {
        _NlSvf__tap (_state._state, value);
    }

    void _NlSvf__tap (_NlSvf_State& _state, int32_t v) noexcept
    {
        _state.tapSel = v;
    }

    void _initialise (NlSvf_State& _state, int32_t& processorID, int32_t sessionID, double frequency) noexcept
    {
        _NlSvf___initialise (_state._state, processorID, sessionID, frequency);
    }

    void _NlSvf___initialise (_NlSvf_State& _state, int32_t& processorID, int32_t sessionID, double frequency) noexcept
    {
    ...
    void _advance (NlSvf_State& _state, NlSvf_IO& _io, int32_t _frames) noexcept
    {
        _NlSvf_IO  ioCopy;
        ...
        for (;;)
        {
            ...
            _NlSvf__main (_state._state, ioCopy);
    ...
    void _NlSvf__main (_NlSvf_State& _state, _NlSvf_IO& _io) noexcept
    {
        float  v0;
        float  bpPrev;
        float  v3;
        ...
        for (;;)
        {
            if (_state.dirty)
            {
                _NlSvf__recompute (_state);
            }
            v0 = _io.in;
            if (_state.rsat > 0.0f)
            {
                bpPrev = _state.bp;
                v0 = (v0 - ((intrinsics::abs (_state.k) * _state.rsat) * (bpPrev - _NlSvf__satRes (bpPrev))));
            }
            v3 = v0 - _state.ic2;
            ...
            if (_state.rsat > 0.0f)
            {
                _state.ic1 = (_state.ic1 + (_state.rsat * (_NlSvf__rail (_state.ic1) - _state.ic1)));
                _state.ic2 = (_state.ic2 + (_state.rsat * (_NlSvf__rail (_state.ic2) - _state.ic2)));
            }
            _state.bp = v1;
            result = {};
            if (_state.tapSel == int32_t {1})
            {
                result = ((v0 - (_state.k * v1)) - v2);
            }
            else
            {
            ...
            _io.out = (_io.out + result);
            return;
        }
    }

    void _NlSvf__recompute (_NlSvf_State& _state) noexcept
    {
        float  sr;
        float  c;
        float  r;
    ...
    float _NlSvf__satRes (float x) noexcept
    {
        float  b;
        float  s;

        b = 0.18f;
        s = 1.0f / _NlSvf__padTanhDeriv (b);
        return (_NlSvf__padTanh (x + b) - _NlSvf__padTanh (b)) * s;
    }

    float _NlSvf__padTanhDeriv (float x) noexcept
    {
        float  x2;
        float  den;
        float  num;
    ...
    float _NlSvf__padTanh (float x) noexcept
    {
        float  x2;
