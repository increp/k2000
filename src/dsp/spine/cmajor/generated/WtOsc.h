...
//==============================================================================
/// Auto-generated C++ class for the 'WtOsc' processor
///
...
struct WtOsc
{
    ...
    static constexpr std::string_view name = "WtOsc";
    ...
    static constexpr uint32_t getEndpointHandleForName (std::string_view endpointName)
    {
        ...
        if (endpointName == "tableIn")    return static_cast<uint32_t> (EndpointHandles::tableIn);
        return 0;
    }

    static constexpr EndpointInfo inputEndpoints[] =
    {
        { 1,  "frequency",  EndpointType::event },
        { 2,  "tableIn",    EndpointType::value }
    };
    ...
    void addEvent_frequency (float event)
    {
        _sendEvent_frequency (cmajState, event);
    }
    ...
    void addEvent (EndpointHandle endpointHandle, uint32_t typeIndex, const unsigned char* eventData)
    {
        ...
        if (endpointHandle == 1)
        {
            ...
            eventData += 4;
            return addEvent_frequency (value);
        }
    }

    void setValue (EndpointHandle endpointHandle, const void* value, int32_t frames)
    {
        if (endpointHandle == 2) return _setValue_tableIn (cmajState, *(Array<float, 256>*) value, frames);
    }
    ...
    WtOsc_State cmajState = {};
    WtOsc_IO cmajIO = {};

    //==============================================================================
    void _sendEvent_frequency (WtOsc_State& _state, float value) noexcept
    {
        _WtOsc__frequency (_state._state, value);
    }

    void _WtOsc__frequency (_WtOsc_State& _state, float v) noexcept
    {
        _state.freq = v;
    }

    void _setValue_tableIn (WtOsc_State& _state, Array<float, 256>& value, int32_t frames) noexcept
    {
        _state._v_tableIn = value;
    }

    void _initialise (WtOsc_State& _state, int32_t& processorID, int32_t sessionID, double frequency) noexcept
    {
        _WtOsc___initialise (_state._state, processorID, sessionID, frequency);
    }

    void _WtOsc___initialise (_WtOsc_State& _state, int32_t& processorID, int32_t sessionID, double frequency) noexcept
    {
    ...
    void _advance (WtOsc_State& _state, WtOsc_IO& _io, int32_t _frames) noexcept
    {
        _WtOsc_IO  ioCopy;
        ...
        for (;;)
        {
            ...
            {
                WtOsc___updateRamps (_state);
            }
            _state._state._v_tableIn = _state._v_tableIn;
            ioCopy = _WtOsc_IO {};
            _WtOsc__main (_state._state, ioCopy);
    ...
    void WtOsc___updateRamps (WtOsc_State& _state) noexcept
    {
    }

    void _WtOsc__main (_WtOsc_State& _state, _WtOsc_IO& _io) noexcept
    {
        float  pos;
        int32_t  idx;
        int32_t  i0;
        ...
        for (;;)
        {
            pos = _state.phase * 256.0f;
            idx = static_cast<int32_t> (pos);
            i0 = intrinsics::modulo (idx, g_n);
            i1 = intrinsics::modulo (i0 + int32_t {1}, g_n);
            frac = pos - static_cast<float> (idx);
            _io.out = (_io.out + ((_state._v_tableIn[std__intrinsics___wrap_256 (i0)] * (1.0f - frac)) + (_state._v_tableIn[std__intrinsics___wrap_256 (i1)] * frac)));
            _state.phase = (_state.phase + (_state.freq / static_cast<float> (1.0 * g__frequency)));
            for (;;)
            {
                ...
                _state.phase = (_state.phase - 1.0f);
            }
            return;
        }
    }

    int32_t std__intrinsics___wrap_256 (int32_t n) noexcept
    {
        return n & int32_t {255};
    }
    ...
    double g__frequency {};
    static constexpr int32_t g_n { int32_t {256} };
